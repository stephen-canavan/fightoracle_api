"""Scrape fights for events before 2016."""
...
# Filter events before 2016
cutoff = datetime(2016, 3, 3)
old_events = [e for e in events if datetime.strptime(e['date'], '%Y-%m-%d') < cutoff]
old_events = sorted(old_events, key=lambda x: x['date'], reverse=True)

logger.info(f"Scraping fights for {len(old_events)} events before 2016")

for i, event in enumerate(old_events, 1):
    event_id = event['ufcstats_event_id']
    logger.info(f"[{i}/{len(old_events)}] Scraping event: {event['name']} ({event['date']})")
